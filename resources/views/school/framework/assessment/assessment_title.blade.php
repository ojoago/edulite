    ...
    <div class="card-body">
        ...
        <div class="tab-content pt-2" id="borderedTabJustifiedContent">
            <div class="tab-pane fade show active" id="bordered-justified-assessment" role="tabpanel" aria-labelledby="assessment-tab">
                ...
                <table class="table table-hover table-striped" id="title-dataTable">
                    <thead>
                        <tr>
                            <th>Title</th>
                            <th>Description</th>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="container">
                    <div class="row">
                        <div class="col-md-7">
                            <form action="" method="post" id="createScoreSettingForm1">
                                @csrf
                    ...
                    <div class="text-center">
                        ...
                        <button id="addMore" type="button" class="btn btn-danger btn-sm btn-small m-3">Add More Row</button>
                        <p class="text-danger titles_pid_error"></p>
                    </div>
                    <div id="settingParamsError">
                        ...
                        <div class="row" id="inputRow">
                            ...
                            <div class="col-md-7">
                                <div class="input-group mb-3">
                                    <input type="text" class="form-control form-control-sm" name="score[]" placeholder="obtainable score">
                                    <span class="input-group-text">Mid-Term?</span>
                                    <input class="custom-check m-1" value="1" name="mid[]" type="checkbox" id="gridCheck2">
...
<script>
    $(document).ready(function() {
        ...
        function titleDropDown(id) {
            $('#titleSelect' + id).select2({
                placeholder: 'Select Title',
                dropdownParent: $('#createScoreSettingModal'),
                ajax: {
                    url: "{{route('load.available.title')}}",
                    dataType: 'json',
                    // Additional AJAX parameters go here; see the end of this chapter for the full code of this example
                    processResults: function(response) {
                        return {
        ...
        $('#addMore').click(function() {
            ...
            $('#settingParams').prepend(
                `
                <div class="row" id ="prependRow${pid}">
                            <div class="col-md-5">
                            ...
                            <div class="col-md-7">
                                <div class="input-group mb-3">
                                    ...
                                    <span class="input-group-text">Mid-Term?</span>
                                    <input class="custom-check m-1" value="1" name="mid[]" type="checkbox" id="gridCheck2">
                                    <i class="bi bi-x-circle-fill text-danger pointer removeRow m-2" id="${pid}"></i>
                                </div>
                                <p class="text-danger score_error"></p>
                            </div>
                        </div>`
        ...
        $('#createScoreSettingBtn').click(function(e) {
            ...
            $('.overlay').show();
            $.ajax({
                url: "{{route('create.school.score.settings')}}",
                type: "POST",
                data: new FormData($('#createScoreSettingForm1')[0]),
                dataType: "JSON",
                processData: false,
                contentType: false,
                beforeSend: function() {
                    $('#createScoreSettingForm1').find('p.text-danger').text('');
                    $('#settingParams').find('p.text-danger').text('');
                    $('#createScoreSettingBtn').prop('disabled', true);
                },
                success: function(data) {
                    console.log(data);
                    $('#createScoreSettingBtn').prop('disabled', false);
                    $('.overlay').hide();
                    if (data.status === 0) {
                        ...
                        alert_toast(data.message, 'success');
                        $('#createScoreSettingForm1')[0].reset();
                    }
                },
                error: function(data) {
                    $('#createScoreSettingBtn').prop('disabled', false);
