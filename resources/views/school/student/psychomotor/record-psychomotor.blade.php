                ...
                <tbody>
                    ...
                    <tr class="studentId" id="{{$student->pid}}">
                        ...
                        <form>
                            @csrf
                            @foreach($psycho as $row)
                            <td scope="col"><input type="number" value="{{getPsychoKeyScore(student:$student->pid,param:$params['param'],key:$row->pid)}}" step="0.01" class="form-control form-control-sm studentPsycho" id="{{$row->pid}}" placeholder="max obtainable {{$row->max_score}}"> </td>
                            @endforeach
                    </tr>
                    @endforeach
                </tbody>
